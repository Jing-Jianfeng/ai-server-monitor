...
app.get('/api/server/gpu', async (req, res) => {
  try {
    ...
    const gpuOutput = await executeCommand(
      `nvidia-smi --query-gpu=${gpuInfoQuery} --format=csv,noheader,nounits`
    );
    ...
    const gpuData = gpuOutput.trim().split('\n').map((line, index) => {
      ...
      return {
        id: parseInt(values[0]) || index,
        name: values[1] || 'Unknown GPU',
        driverVersion: values[2] || 'N/A',
        temperature: {
          gpu: parseFloat(values[3]) || 0,
          memory: parseFloat(values[4]) || 0
        },
        utilization: {
          gpu: parseFloat(values[5]) || 0,
          memory: parseFloat(values[6]) || 0
        },
        memory: {
          used: parseFloat(values[7]) || 0,
          total: parseFloat(values[8]) || 0,
          free: parseFloat(values[9]) || 0,
          percentage: parseFloat(values[8]) > 0 ? Math.round((parseFloat(values[7]) / parseFloat(values[8])) * 100) : 0
        },
        power: {
          draw: parseFloat(values[10]) || 0,
          limit: parseFloat(values[11]) || 0,
          percentage: parseFloat(values[11]) > 0 ? Math.round((parseFloat(values[10]) / parseFloat(values[11])) * 100) : 0
        },
        clocks: {
          graphics: {
            current: parseFloat(values[12]) || 0,
            max: parseFloat(values[14]) || 0
          },
          memory: {
            current: parseFloat(values[13]) || 0,
            max: parseFloat(values[15]) || 0
          }
        },
        fan: {
          speed: parseFloat(values[16]) || 0
        },
        performance: {
          state: values[17] || 'P0',
          stateNumeric: parseInt(values[17]?.replace('P', '')) || 0
        },
        computeMode: values[18] || 'Default',
        busId: values[19] || 'N/A',
        display: {
          mode: values[20] || 'Enabled',
          active: values[21] || 'Disabled'
        }
      };
    });
    ...
    // If nvidia-smi not available, return empty data with error info
    res.json({
      gpus: [],
      processes: [],
      error: 'NVIDIA GPU monitoring not available',
      timestamp: new Date().toISOString()
    });
  }
});
